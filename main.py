...
from database import insert_data, create_table, fetch_data, delete_rows
...
def sync_google_sheets_with_db():
    try:
        ...
        #print(f"Filtered sheet data: {sheet_data}")

        db_data = fetch_data()
        '''------------------------------------
          Handle Insertions/Updates in Postgres 
          -------------------------------------'''
...
def update_gsheet():
    try:
        ...
        header = sheet_data[0]
        sheet_data = sheet_data[1:]  # Skip header
        ...
        '''-----------------------------------------
          Handle Insertions/Updates in Google Sheets 
          ------------------------------------------'''
        rows_to_update = []
